...
run.init(folder="/FNO_MHD/pre_IAEA", tags=['Multi-Blobs', 'MultiVariable', "Z_Li", "Skip-connect", "Diff", "Recon", "Error Train"], metadata=configuration)
...
model_earlier = FNO_multi(modes, modes, width_vars, width_time)
model_earlier.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_violent-spot.pth', map_location=torch.device('cpu'))) #500 Epochs 
# model_earlier.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_violent-spot.pth')) #500 Epochs 
model_earlier.to(device)

#Loading the trained error model
model_error = FNO_multi(modes, modes, width_vars, width_time)
model_error.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_grouchy-creek.pth', map_location=torch.device('cpu'))) #Model trained on the errors
model_error.to(device)
...
pred_set_error = torch.zeros(test_u.shape)
index = 0
with torch.no_grad():
    for xx, yy in tqdm(test_loader):
        ...
        for t in range(0, T, step):
            y = yy[..., t:t + step]
            out = model_earlier(xx)
            out_error = model_error(xx_error)

            if t == 0:
                pred_error = out_error
                pred = out
            else:
                pred = torch.cat((pred, out), -1)
                pred_error = torch.cat((pred_error, out_error), -1)

            xx = torch.cat((xx[..., step:], out), dim=-1)
            xx_error = torch.cat((xx_error[..., step:], out_error), dim=-1)
        t2 = default_timer()
