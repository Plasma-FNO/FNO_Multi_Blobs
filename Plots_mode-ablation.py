...
model_2 = FNO_multi(2, 2, width_vars, width_time)
model_2.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_strong-rectangle.pth', map_location=torch.device('cpu')))

model_4 = FNO_multi(4, 4, width_vars, width_time)
model_4.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_international-harrier.pth', map_location=torch.device('cpu')))

model_8 = FNO_multi(8, 8, width_vars, width_time)
model_8.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_avocado-column.pth', map_location=torch.device('cpu')))

model_16 = FNO_multi(16, 16, width_vars, width_time)
model_16.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_sour-goalie.pth', map_location=torch.device('cpu')))

model_32 = FNO_multi(32, 32, width_vars, width_time)
model_32.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_respectful-bar.pth', map_location=torch.device('cpu')))

models = [model_2, model_4, model_8, model_16, model_32]
...
for model in models:
    ...
    model_preds.append(pred_set)

# %% 

preds_encoded = model_preds
preds = []
...
for ii in range(len(models)):
    err = [] 
    for it in range(T):
        err.append(torch.abs(preds_encoded[ii][...,it] - test_u_encoded[...,it]).mean())
